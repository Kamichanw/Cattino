...
def create(
    ...
    for ex_arg in extra_args:
        if task_name is not None:
            ...
            fullnames.append(formatted_name)
        else:
...
def resume(all: bool, name: Optional[str]):
    ...
    response = BackendRequest.resume(name, use_regex=all)
    print_response(
        response,
        lambda success: (
...
def kill(all: bool, force: bool, name: Optional[str]):
    ...
    response = BackendRequest.kill(name, force=force, use_regex=all)
    print_response(
        response,
        lambda success: (
...
def remove(all: bool, name: Optional[str]):
    ...
    response = BackendRequest.remove(name, use_regex=all)
    print_response(
        response,
        lambda success: (
    ...
    if "." in setting_or_task_attr:
        ...
        try:
            ...
            if settings.all_settings[key] != old_value:
                ...
                click.echo(f"Setting {setting} has already to be set to {value}.")
