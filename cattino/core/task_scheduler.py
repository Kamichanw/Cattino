...
class TaskScheduler:
    ...
    async def _execute_task(self, task: Task):
        def is_first(t: AbstractTask):
            ...
            # check all subtasks, is_first can be True only if
            #   - # remaining == # total
            ...
            def is_subgroup_first(sub_g: TaskGroup) -> bool:
                try:
                    sub_g_info = self._group_info.get(next(iter(sub_g)))
                    # if sub_g_info is None, the task was cleaned up, so it's not "first".
                    # Otherwise, check its cached is_first flag.
                    return sub_g_info.is_first if sub_g_info else False
                except StopIteration:
                    # an empty group doesn't violate the "is_first" condition.
                    return True
